...
// 실사용 중 발견한 엣지케이스
//
// 스테이징 앱을 직접 써보다가, 투표 선택지에 같은 문구를 두 번 입력하면
// (예: "치킨" / "치킨") 어느 쪽을 눌러도 같은 선택지로 처리되는 것처럼
// 보이는 현상을 발견했다. 원인은 득표 집계 로직이 선택지를 "텍스트"로
// 구분하고 있었기 때문 — 텍스트가 같으면 서로 다른 선택지라도 득표가
// 뒤섞인다. 아래 TC로 고정하고, 앱 코드는 선택지를 인덱스로 구분하도록
// 수정했다.
// ────────────────────────────────────────────────────────────
